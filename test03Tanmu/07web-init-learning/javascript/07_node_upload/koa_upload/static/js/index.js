        ...
        uploadAction({
            success: function (result) {
                ...
                if (result && result.success && result.data && result.data.pictureUrl) {
                    let reg = /\.(png|jpg|svg|gif)$/
                    let reg2 = /\.(mp4|ogg|mov|flv)$/
                    if (reg.test(result.data.pictureUrl)) {
                        previewElem.innerHTML = '<img src="' + result.data.pictureUrl + '" width="200" height="200" />'
                    } else {
                        previewElem.innerHTML = `<video src="${result.data.pictureUrl}" width="200" height="200" controls="controls" autoplay="autoplay" loop="loop" class="video-show"></video>`
                    }
