...
let showChatRoom = () => {
    ...
    socket.on('loginUser', (data) => {
        ...
        let tipUserStr = `<img src="${data.imgpath}" alt="touxiang" class="user-tx" width="20" height="20" /> <span>${data.userName}</span>`
...
let showUser = (data) => {
    ...
    // console.log(data.userName, data.imgpath)
...
let userMsg = (data) => {
    ...
    console.log(userSelf, receiveUsers)

    userMsgArrs.map( (con, i) => {

        con.onclick = function() {
            console.log(this)
            console.log(receiveUsers[i])
        }
    })
}
